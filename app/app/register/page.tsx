...
function ProtonIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M3 5C3 3.89543 3.89543 3 5 3H14L21 10V19C21 20.1046 20.1046 21 19 21H5C3.89543 21 3 20.1046 3 19V5Z" fill="#6D4AFF" opacity="0.9"/>
...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const globalMsg = data?.ui?.messages?.[0]?.text || data?.error?.message || "Registration failed"
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-10" style={{ background: "#1a1a2e" }}>
      <div className="w-full max-w-md rounded-2xl border p-8" style={{ background: "#16213e", borderColor: "#2a2a4a" }}>

        {/* Logo */}
        <div className="flex items-center gap-2 mb-8">
          <span className="w-8 h-8 rounded flex items-center justify-center text-white text-sm font-bold" style={{ background: "#7F77DD" }}>R</span>
          <span className="font-bold text-lg" style={{ color: "#e0e0e0" }}>REXFORM</span>
          <span className="font-bold text-lg" style={{ color: "#7F77DD" }}>· Notes</span>
        </div>

        <h1 className="text-2xl font-bold mb-1" style={{ color: "#e0e0e0" }}>Create your account</h1>
        <p className="text-sm mb-6" style={{ color: "#8892a4" }}>Your personal knowledge base awaits</p>
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <span className="text-xs" style={{ color: "#4a5568" }}>or register with email</span>
          <div className="flex-1 h-px" style={{ background: "#2a2a4a" }} />
        </div>

        {globalError && (
          <div className="mb-4 px-4 py-3 rounded-lg text-sm border" style={{ background: "#2d1a1a", borderColor: "#7a2020", color: "#f87171" }}>
            {globalError}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium mb-1.5" style={{ color: "#8892a4" }}>First name</label>
              <input
                type="text"
                value={firstName}
              ...
              />
              {errors["traits.name.first"] && <p className="text-xs mt-1" style={{ color: "#f87171" }}>{errors["traits.name.first"]}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium mb-1.5" style={{ color: "#8892a4" }}>Last name</label>
              <input
                type="text"
                value={lastName}
              ...
              />
              {errors["traits.name.last"] && <p className="text-xs mt-1" style={{ color: "#f87171" }}>{errors["traits.name.last"]}</p>}
            </div>
          </div>

          <div>
            <label className="block text-sm font-medium mb-1.5" style={{ color: "#8892a4" }}>Email</label>
            <input
              type="email"
              value={email}
            ...
            />
            {errors["traits.email"] && <p className="text-xs mt-1" style={{ color: "#f87171" }}>{errors["traits.email"]}</p>}
          </div>

          <div>
            <label className="block text-sm font-medium mb-1.5" style={{ color: "#8892a4" }}>Password</label>
            <input
              type="password"
              value={password}
        ...
        <p className="mt-6 text-center text-sm" style={{ color: "#8892a4" }}>
          ...
          <Link href="/login" className="font-medium hover:underline" style={{ color: "#7F77DD" }}>Sign in</Link>
        </p>
      </div>
    </div>
  )
