...
| `SSO_ISSUER_URL` | Optional | Central IAM OIDC issuer (rexform-ory-dev gateway). No trailing slash. Set all three SSO vars together or none — the login/register SSO button appears automatically when the provider is registered (runtime check, no rebuild needed) | `https://gateway-production-1b0d.up.railway.app` |
| `SSO_CLIENT_ID` | Optional | OAuth2 client ID registered in the central Hydra | `rexform-notes` |
